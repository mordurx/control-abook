...
{   include("check_end.php");
    ...
    //  //si es el primer cuento que leo
    //  $sql="SELECT count(id_orden)as id_orden FROM Order_Cuento WHERE id_usuario='$user'";
    //     if (!$resultado = $con->query($sql)) 
    // {
    //     $error=mysqli_error($con);
    //     echo $error;
    ...
    // $arr_orden = $resultado->fetch_assoc();
    // $id_orden=$arr_orden['id_orden'];

    
    // if ($id_orden==0) {
    //     # code...
    //      $sql="INSERT ignore INTO Orden_Cuento (id_orden,id_cuento,id_usuario) values (1,$id_cuento,$user)";
    //             if (!$resultado = $con->query($sql)) 
    //             {
    //                 $error=mysqli_error($con);
    //                 //echo $error;
    ...
    // else
    // {
    // if ($id_cuento==$id_last_cuento and $veces_leido==$veces_leido_cuento) {

    //     $sql="SELECT id_orden FROM Order_Cuento WHERE id_usuario='$user' ORDER BY id_orden DESC limit 1";
    //     if (!$resultado = $con->query($sql)) 
    //         {
    //             $error=mysqli_error($con);
    //             echo $error;
    ...
    //         $arr_orden = $resultado->fetch_assoc();
    //         $last_id_orden=$arr_orden['id_orden'];

    //     //update orden a pregunta
    //     $sql1="update Respuesta_pregunta set order_resp=$last_id_orden where nombre_usuario='$user' and id_pregunta=$id_pregunta";
    //         if (!$resultado = $con->query($sql1)) 
    //         {
    //             $error=mysqli_error($con);
    //             echo $error;
